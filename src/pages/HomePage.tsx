...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fa] font-sans pb-10">
        ...
        {/* 手动编号清洗抽屉 */}
        <div 
          className={`fixed top-0 right-0 w-[420px] h-full bg-white shadow-xl border-l border-gray-200 z-[9999] flex flex-col transition-transform duration-300 ease-in-out ${
            isCleanDrawerOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
          style={{ zIndex: 9999 }}
        >
          {/* Header */}
                  ...
                  <div className="divide-y divide-gray-50">
                    {items.map(item => {
                      ...
                      return (
                        <div
                          key={item.index}
                          className={`group flex items-center gap-3 p-2 rounded-lg transition-colors cursor-default border ${
                            isChecked
                        ...
                        >
                          ...
                          {/* 右侧排除/撤销按钮 */}
                          <button 
                            className={`px-2 py-1 text-xs rounded border transition-all shadow-sm ${
                              isChecked
